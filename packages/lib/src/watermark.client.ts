...
export interface WatermarkOptions {
  ...
  pica?: import('pica').Pica; // pass pica instance if resizing
}
...
export async function applyWatermark(
  ...
  if (imageSrc) {
    ...
    wmW *= scale;
    wmH *= scale;
    const [x, y] = getPosition(canvas.width, canvas.height, wmW, wmH, position, margin);
  ...
  if (text) {
    ...
    const [x, y] = getPosition(canvas.width, canvas.height, textW, textH, position, margin);
