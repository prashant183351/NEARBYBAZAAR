...
export function computeWatermarkSignature({ text, secret }: { text: string; secret: string }): string {
    return crypto.createHash('sha256').update(text + secret).digest('hex').slice(0, 8);
}
...
export async function verifyWatermark(
    ...
    const expectedSig = computeWatermarkSignature({ text: options.expectedText, secret: options.secret });
    // 3. Compare
