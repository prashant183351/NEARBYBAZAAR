...
it('should handle pagination of margin rules', async () => {
  ...
  // Paginate: page 1
  const page1 = await MarginRule.find({ vendorId: vendor._id }).limit(10).skip(0);

  // Paginate: page 2
  const page2 = await MarginRule.find({ vendorId: vendor._id }).limit(10).skip(10);
...
it('should prevent orders to suspended suppliers', async () => {
  ...
  await expect(pushOrderToSupplier(order, supplier)).rejects.toThrow('Supplier is suspended');
});
...
it('should handle negative costs', async () => {
  ...
  await expect(calculatePrice(-10, marginRule)).rejects.toThrow('Cost must be positive');
});
```
