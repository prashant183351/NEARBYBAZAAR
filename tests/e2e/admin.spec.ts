...
test.describe('Admin PWA - Basic Navigation', () => {
  ...
  test('should navigate to orders page', async ({ page }) => {
    ...
    const ordersContent = page.locator('text=/orders|transactions/i, [data-testid="orders"]').first();
  ...
  test('should have navigation menu', async ({ page }) => {
    ...
    // Common admin links
    const hasUsers = await page.locator('text=/users/i').isVisible().catch(() => false);
    const hasOrders = await page.locator('text=/orders/i').isVisible().catch(() => false);
    const hasReports = await page.locator('text=/reports/i').isVisible().catch(() => false);
