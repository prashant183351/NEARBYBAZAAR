  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Set up a new advertising campaign for your products</p>
      </div>

      {/* Progress Steps */}
      <div className="mb-8">
        <div className="flex items-center justify-between">
          {['Basic Info', 'Budget & Bid', 'Targeting', 'Review'].map((label, index) => {
            ...
            return (
              <div key={label} className="flex items-center flex-1">
                ...
                {index < 3 && (
                  <div
                    className={`flex-1 h-1 mx-4 ${isCompleted ? 'bg-green-500' : 'bg-gray-200'}`}
                  ></div>
                )}
              </div>
            );
      ...
      <div className="bg-white rounded-lg shadow p-8">
        ...
        {step === 2 && (
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Bid Type *</label>
              <div className="grid grid-cols-2 gap-4">
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, bidType: 'cpc' })}
                  className={`p-4 border-2 rounded-lg text-left ${
                    formData.bidType === 'cpc' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'
                  }`}
                >
                  <p className="font-bold">Cost Per Click (CPC)</p>
                  <p className="text-sm text-gray-600 mt-1">Pay only when someone clicks your ad</p>
                </button>
                <button
                  type="button"
                  onClick={() => setFormData({ ...formData, bidType: 'cpm' })}
                  className={`p-4 border-2 rounded-lg text-left ${
                    formData.bidType === 'cpm' ? 'border-blue-600 bg-blue-50' : 'border-gray-200'
                  }`}
                >
                  <p className="font-bold">Cost Per Mille (CPM)</p>
                  <p className="text-sm text-gray-600 mt-1">Pay per 1000 impressions</p>
                </button>
              </div>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <p className="text-sm text-blue-800">
                <strong>Estimated Performance:</strong> With ₹{formData.totalBudget} budget at ₹
                {formData.bidAmount} per {formData.bidType === 'cpc' ? 'click' : '1000 impressions'}
                , you could get approximately{' '}
                {formData.bidType === 'cpc'
                  ? Math.floor(formData.totalBudget / formData.bidAmount)
                  : Math.floor((formData.totalBudget / formData.bidAmount) * 1000)}{' '}
