...
export default function CampaignsDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/v1/ad-dashboard/vendor/summary?daysBack=${dateRange}`, {
        headers: {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Ad Campaigns Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-3xl font-bold text-gray-900 mt-2">{data.summary.totalCampaigns}</p>
          <p className="text-sm text-gray-500 mt-1">{data.summary.activeCampaigns} active</p>
        </div>
          ...
          <div>
            ...
            <div className="mt-2 w-full bg-gray-200 rounded-full h-2">
              <div
                ...
                style={{
                  width: `${
                    (data.budgetAnalysis.totalSpent / data.budgetAnalysis.totalBudget) * 100
                  }%`,
                }}
              ></div>
            </div>
      ...
      {data.topCampaigns.length > 0 && (
        <div className="bg-white rounded-lg shadow p-6 mb-8">
          <h2 className="text-xl font-bold text-gray-900 mb-4">Top Performing Campaigns</h2>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {data.campaigns.map((campaign) => (
                <tr key={campaign.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <p className="text-sm text-gray-900">₹{campaign.bidAmount}</p>
                    <p className="text-xs text-gray-500 uppercase">{campaign.bidType}</p>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <p className="text-sm text-gray-900">
                      ₹{campaign.spentTotal.toFixed(2)} / ₹{campaign.totalBudget.toFixed(2)}
                    </p>
                    <div className="mt-1 w-24 bg-gray-200 rounded-full h-1.5">
                      <div
                        className="bg-blue-600 h-1.5 rounded-full"
                        style={{
                          width: `${(campaign.spentTotal / campaign.totalBudget) * 100}%`,
                        }}
                      ></div>
                    </div>
                  </td>
