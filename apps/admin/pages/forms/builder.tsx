...
export default function FormBuilder() {
    ...
    return (
        <div style={{ padding: 32 }}>
            ...
            <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd} onDragStart={() => setDragActive(true)}>
                <SortableContext items={fields.map(f => f.id)} strategy={verticalListSortingStrategy}>
                    <ul style={{ listStyle: 'none', padding: 0 }}>
                        {fields.map(field => (
                            <li key={field.id} style={{ marginBottom: 12, background: dragActive ? '#f0f0f0' : '#fff', padding: 12, border: '1px solid #ddd', borderRadius: 4 }}>
            ...
            <button onClick={handleSave} style={{ marginTop: 24 }}>Save Form</button>
        </div>
    );
}
