 ...
 */
async function calculateLateShipmentRate(vendorId: string, startDate: Date, endDate: Date): Promise<number> {
 ...
 */
async function calculateCancellationRate(vendorId: string, startDate: Date, endDate: Date): Promise<number> {
	const totalOrders = await Order.countDocuments({
		vendor: vendorId,
		createdAt: { $gte: startDate, $lte: endDate },
	});
 ...
 */
function determineStatus(metrics: Pick<ReputationMetrics, 'orderDefectRate' | 'lateShipmentRate' | 'cancellationRate'>): ReputationMetrics['status'] {
	const { odr, lateShipment, cancellation } = THRESHOLDS;
	
	// Critical if any metric exceeds critical threshold
	if (metrics.orderDefectRate >= odr.critical || 
	...
	if (metrics.orderDefectRate >= odr.warning || 
	...
	if (metrics.orderDefectRate > odr.excellent || 
...
export async function evaluateVendorStanding(vendorId: string): Promise<{
	...
	if (metrics.status === 'critical') {
		...
		if (metrics.lateShipmentRate >= THRESHOLDS.lateShipment.critical) {
			reasons.push(`Late shipment ${metrics.lateShipmentRate}% exceeds ${THRESHOLDS.lateShipment.critical}%`);
		}
		if (metrics.cancellationRate >= THRESHOLDS.cancellation.critical) {
			reasons.push(`Cancellation ${metrics.cancellationRate}% exceeds ${THRESHOLDS.cancellation.critical}%`);
		}
		return {
			action: 'suspend',
			reason: reasons.join('; '),
	...
	if (metrics.status === 'needs_improvement') {
		...
		if (metrics.lateShipmentRate >= THRESHOLDS.lateShipment.warning) {
			reasons.push(`Late shipment ${metrics.lateShipmentRate}% exceeds ${THRESHOLDS.lateShipment.warning}%`);
		}
		if (metrics.cancellationRate >= THRESHOLDS.cancellation.warning) {
			reasons.push(`Cancellation ${metrics.cancellationRate}% exceeds ${THRESHOLDS.cancellation.warning}%`);
		}
		return {
			action: 'warning',
			reason: reasons.join('; '),
