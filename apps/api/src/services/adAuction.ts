 ...
 */
function calculateRelevanceScore(campaign: AdCampaignType, context: AuctionContext): number {
  let score = 0;
  ...
  if (keywords.length > 0 && campaign.keywords.length > 0) {
    ...
    const keywordMatchRatio =
      matchedKeywords.length / Math.max(keywords.length, campaign.keywords.length);
 ...
 */
export async function getSearchAds(searchQuery: string, limit: number = 2): Promise<AdResult[]> {
 ...
 */
export async function getCategoryAds(categoryId: string, limit: number = 3): Promise<AdResult[]> {
  return runAdAuction({
    placement: 'category',
    categoryId,
    limit,
...
export async function estimateCampaignPerformance(
  ...
  const avgCTR =
    competitors.length > 0
      ? competitors.reduce((sum: number, c: AdCampaignType) => sum + (c.ctr || 0), 0) /
        competitors.length
      : 0.02; // Default 2%
