...
export function recordOrderAttempt(req: Request) {
  ...
  ipOrderTimestamps[ipStr] = (ipOrderTimestamps[ipStr] || []).filter((ts: number) => now - ts < 10 * 60 * 1000); // last 10 min
  ipOrderTimestamps[ipStr].push(now);
  // Device velocity
  const device = getDeviceFingerprint(req);
  deviceOrderTimestamps[device] = (deviceOrderTimestamps[device] || []).filter((ts: number) => now - ts < 10 * 60 * 1000);
...
export function isVelocityHigh(req: Request, maxPer10min = 5): boolean {
  ...
  const device = getDeviceFingerprint(req);
  const ipStr = String(ip);
  return (ipOrderTimestamps[ipStr]?.length || 0) > maxPer10min || (deviceOrderTimestamps[device]?.length || 0) > maxPer10min;
}

export function getRiskScore({ velocity, suspiciousDevice }: { velocity: boolean; suspiciousDevice: boolean }): number {
  let score = 0;
...
export function isSuspiciousDevice(device: string): boolean {
  ...
  const now = Date.now();
  const count = (deviceOrderTimestamps[device] || []).filter(ts => now - ts < 24 * 60 * 60 * 1000).length;
  return count > 10;
}
...
export async function shouldHoldPayout(vendorId: string, payoutAmount: number) {
	...
	const recentDisputes = await Dispute.countDocuments({ vendor: vendorId, status: 'open', createdAt: { $gte: new Date(Date.now() - 14 * 24 * 60 * 60 * 1000) } });
