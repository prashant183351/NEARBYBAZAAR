...
export function evaluateEscalation(metrics: ReputationMetrics): EscalationDecision {
	...
	ESCALATION_RULES.forEach(rule => {
		...
		if (metricValue >= rule.permanentBlockThreshold) {
			violations.push(`${metricName}: ${metricValue}% (threshold: ${rule.permanentBlockThreshold}%)`);
...
export async function createVendorAction(
	...
	const expiresAt = actionType === 'temp_suspend' 
 ...
 */
export async function getVendorsRequiringAction(): Promise<Array<{
	vendor: any;
