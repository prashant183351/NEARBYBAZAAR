...
export class SupplierRateLimiter {
   ...
   */
  async checkAndRecord(supplierId: string, config?: RateLimitConfig): Promise<RateLimitResult> {
   ...
   */
  async getStatus(supplierId: string, config?: RateLimitConfig): Promise<RateLimitResult> {
    const limits = config || this.defaultConfig;
    const now = Date.now();
    const windowStart = now - limits.windowMs;
    const key = `rate_limit:supplier:${supplierId}`;
...
export class RateLimitRetryQueue {
   ...
   */
  async enqueue(supplierId: string, requestData: any, retryAfter: number): Promise<void> {
  ...
  async getReadyRequests(supplierId: string, limit = 10): Promise<any[]> {
    ...
    const results = await this.redis.zrangebyscore(key, 0, now, 'LIMIT', 0, limit);
