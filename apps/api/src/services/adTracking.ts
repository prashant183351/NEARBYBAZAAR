...
export async function recordClick(data: ClickData): Promise<TrackingResult> {
  try {
    ...
    const charged = await chargeVendorWallet(String(campaign.vendor), cost, String(campaign._id));
...
export async function detectFraudPatterns(campaignId: string, hours: number = 24) {
  ...
  Object.entries(clicksBySession).forEach(([sessionId, sessionClicks]) => {
    if (sessionClicks.length > 5) {
      ...
      if (conversions === 0) {
        patterns.push(
          `Suspicious: ${sessionClicks.length} clicks from session ${sessionId} with no conversions`,
        );
      }
    }
  });
  ...
  const conversionRate =
    clicks.length > 0 ? clicks.filter((c) => c.convertedToOrder).length / clicks.length : 0;

  if (clicks.length > 50 && conversionRate < 0.001) {
    patterns.push(
      `Suspicious: Very low conversion rate (${(conversionRate * 100).toFixed(2)}%) with ${clicks.length} clicks`,
    );
  }
