...
export function buildWarrantySnapshot(
  product: Pick<ProductType, 'warranty'>,
  startDate: Date,
): OrderLineItemWarranty | null {
...
export function resolveWarrantyStatus(
  warranty: OrderLineItemWarranty | undefined,
...
export async function scheduleWarrantyExpiryReminder({
  orderId,
  lineItemId,
  userId,
  warranty,
  bufferDays,
}: WarrantyReminderJob): Promise<void> {
  try {
    ...
    const reminderBuffer =
      typeof bufferDays === 'number'
        ? bufferDays
        : Number(process.env.WARRANTY_REMINDER_DAYS_BEFORE || 7);
    ...
    logger.info(
      { orderId, lineItemId, expiryDate, daysBuffer },
      'Scheduled warranty expiry reminder',
    );
  } catch (error) {
