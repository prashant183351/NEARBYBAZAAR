...
export async function scheduleRenewalReminders() {
    ...
    const subs = await Subscription.find({ status: 'active', endDate: { $gte: now, $lte: fiveDaysLater } });
    for (const sub of subs) {
        ...
        if (sub.endDate) {
            ...
            if (delay > 0) {
                await emailQueue.add('renewalReminder', {
                    to: vendor.email,
                    subject: getI18nTemplate('renewal.subject', vendor.language || 'en', { plan: sub.plan }),
                    body: getI18nTemplate('renewal.body', vendor.language || 'en', { plan: sub.plan, endDate: sub.endDate }),
                }, { delay });
            }
        }
    }
