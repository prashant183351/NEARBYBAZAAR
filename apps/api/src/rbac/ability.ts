 ...
 */
export function can(user: UserClaims, action: Action, resource: Resource, ctx: AbilityContext = {}): boolean {
    ...
    if (user.role === 'vendor') {
        ...
        if ((resource === 'product' || resource === 'service' || resource === 'classified') && ['create', 'update', 'delete'].includes(action)) {
            return !!owns;
        }
    ...
    if (user.role === 'user') {
        ...
        if (resource === 'dispute' && (action === 'create' || action === 'read' || action === 'update')) {
