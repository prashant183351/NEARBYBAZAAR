...
export async function run() {
  ...
  await Vendor.collection
    .createIndex({ slug: 1 }, { unique: true, name: 'uniq_vendor_slug' })
    .catch(() => {});
  ...
  for await (const doc of cursor as any) {
    const base =
      generateSlug(doc.name) || generateSlug(doc.email) || `vendor-${doc._id.toString().slice(-6)}`;
    // Gather existing similar slugs to dedupe
    const regex = new RegExp(`^${base}(?:-\\d+)?$`, 'i');
    const others = await Vendor.find({ slug: { $regex: regex }, _id: { $ne: doc._id } })
      .select('slug')
      .lean();
