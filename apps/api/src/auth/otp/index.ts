...
router.post('/request', rateLimiters.strict, sanitize({ body: { email: 'plain' } }), validate(otpRequestSchema), async (req, res) => {
...
router.post('/verify', sanitize({ body: { email: 'plain', code: 'plain' } }), validate(otpVerifySchema), async (req, res) => {
...
router.post('/totp/enable', sanitize({ body: { userId: 'plain' } }), validate(totpEnableSchema), async (req, res) => {
...
router.post('/totp/verify', sanitize({ body: { userId: 'plain', token: 'plain' } }), validate(totpVerifySchema), async (req, res) => {
...
router.post('/totp/login', sanitize({ body: { userId: 'plain', token: 'plain' } }), validate(totpLoginSchema), async (req, res) => {
  const { userId, token } = req.body;
  const user = await User.findById(userId);
  if (!user || user.deleted) return res.status(404).json({ error: 'User not found' });
...
const backupGenerateSchema = z.object({ userId: z.string(), count: z.number().min(1).max(20).default(10) });
router.post('/backup/generate', sanitize({ body: { userId: 'plain', count: 'plain' } }), validate(backupGenerateSchema), async (req, res) => {
...
router.post('/backup/verify', sanitize({ body: { userId: 'plain', code: 'plain' } }), validate(backupVerifySchema), async (req, res) => {
  ...
  let arr: string[] = [];
  try { arr = JSON.parse(raw); } catch { return res.status(500).json({ error: 'Corrupted backup store' }); }
  const h = hash(code);
