...
const marginRuleBaseSchema = z.object({
    supplierId: z.string().regex(/^[0-9a-fA-F]{24}$/, 'Invalid supplier ID').optional(),
...
router.post('/', async (req, res) => {
    try {
        ...
        const filter: any = { vendorId, active: true };
        ...
        const rule = await MarginRuleModel.create({
            vendorId,
            supplierId: validatedData.supplierId ? new Types.ObjectId(validatedData.supplierId) : undefined,
