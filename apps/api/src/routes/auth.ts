...
import { generateRefreshToken, getRefreshData, rotateRefreshToken, storeRefreshToken, revokeRefreshToken } from '../auth/tokens';
...
router.post('/signup', rateLimiters.sensitive, sanitize({ body: { email: 'plain', password: 'plain', fingerprint: 'plain' } }), validate(signupSchema), async (req, res) => {
	...
	res.json({ token: jwt, refreshToken: refresh, user: { id: user.id, email: user.email, name: user.name, role: user.role } });
});
...
router.post('/login', rateLimiters.sensitive, sanitize({ body: { email: 'plain', password: 'plain', fingerprint: 'plain' } }), validate(loginSchema), async (req, res) => {
...
router.post('/refresh', sanitize({ body: { refreshToken: 'plain' } }), validate(refreshSchema), async (req, res) => {
	...
	const newRefresh = await rotateRefreshToken(refreshToken, user.id, req.headers['user-agent'] as string | undefined);
	const jwt = signJwt({ id: user.id, email: user.email, role: user.role as any });
	res.json({ token: jwt, refreshToken: newRefresh });
});

const logoutSchema = z.object({ refreshToken: z.string().min(1) });
router.post('/logout', sanitize({ body: { refreshToken: 'plain' } }), validate(logoutSchema), async (req, res) => {
	const { refreshToken } = req.body;
	const data = await getRefreshData(refreshToken);
...
router.post('/password-reset', rateLimiters.sensitive, sanitize({ body: { email: 'plain', fingerprint: 'plain' } }), validate(passwordResetSchema), async (req, res) => {
