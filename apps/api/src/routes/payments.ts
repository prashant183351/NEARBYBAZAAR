...
router.post(
  ...
  async (req, res) => {
    ...
    if (!pi)
      return res
        .status(404)
        .json({ success: false, error: { message: 'PaymentIntent not found' } });
    if (pi.gateway !== PaymentGateway.PHONEPE)
      return res.status(400).json({ success: false, error: { message: 'Wrong gateway' } });
    if (
      !(
        pi.status === PaymentStatus.PENDING ||
        pi.status === PaymentStatus.PROCESSING ||
        pi.status === PaymentStatus.REQUIRES_ACTION
      )
    ) {
      return res
        .status(400)
        .json({ success: false, error: { message: `Cannot init in status ${pi.status}` } });
    }
...
router.post('/phonepe/callback', async (req, res) => {
  ...
  if (parsed.data.success) {
    ...
    pi.gatewayError = {
      ...
      message: parsed.data.message || 'Payment failed',
    } as any;
  }
  await pi.save();
