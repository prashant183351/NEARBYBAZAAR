...
export async function generateTestReputationData(vendorId: string, totalOrders: number = 100) {
  ...
  for (let i = 0; i < totalOrders; i++) {
    ...
    if (status !== 'cancelled') {
      ...
      if (isLate) {
        ...
        shippedAt = new Date(
          expectedDispatchDate.getTime() + (3 + Math.random() * 2) * 24 * 60 * 60 * 1000,
        );
      } else {
  ...
  console.log(
    `   - ${orders.filter((o) => o.status === 'refunded' || o.status === 'returned' || o.hasDispute).length} defective orders`,
  );
  console.log(
    `   - ${orders.filter((o) => o.shippedAt && o.expectedDispatchDate && o.shippedAt > o.expectedDispatchDate).length} late shipments`,
  );
  console.log(
    `   - ${orders.filter((o) => o.status === 'cancelled' && (o.cancelledBy === 'vendor' || o.cancellationReason === 'out_of_stock')).length} vendor cancellations`,
  );
