...
export async function seedDevData() {
  ...
  if (vendor) {
    await Vendor.updateOne(
      { _id: vendor._id },
      { $set: { name: vendorName, owner: vendorOwnerId } },
    );
  ...
  for (const p of products) {
    ...
    if (existing) {
      await Product.updateOne(
        { _id: existing._id },
        { $set: { description: p.description, price: p.price, currency: p.currency } },
      );
  ...
  // Service
  const service = {
    name: 'Mobile Repair Basic',
    description: 'Screen and battery replacement service.',
    price: 499,
    currency: 'INR',
    duration: 60,
  };
