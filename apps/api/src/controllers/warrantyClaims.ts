...
export async function createWarrantyClaim(req: Request, res: Response) {
    ...
    const order = await Order.findOne({ _id: new Types.ObjectId(orderId), user: new Types.ObjectId(user.id), deleted: false });
    ...
    const vendorIdHex = (order as any).vendor ? ((order as any).vendor as Types.ObjectId).toHexString() : undefined;
