	...
	try {
		...
		const budgetAnalysis = {
			...
			remainingBudget: campaigns.reduce(
				(sum, c) => sum + (c.totalBudget - c.spentTotal),
				0
			),
		...
		const platformMetrics = {
			...
			averageRevenuePerClick:
				revenue.totalClicks > 0
					? (revenue.totalRevenue / revenue.totalClicks).toFixed(2)
					: '0.00',
		};
		...
		const comparison = campaigns.map(campaign => {
			...
			return {
				...
				metrics: {
					...
					conversions: campaignClicks.reduce(
					...
					conversionRate:
						campaign.clicks > 0
							? (
									(campaignClicks.reduce((sum, d) => sum + d.conversions, 0) /
										campaign.clicks) *
									100
							  ).toFixed(2)
							: '0.00',
