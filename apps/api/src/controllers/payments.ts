...
export async function refundOrder(req: Request, res: Response) {
    const { orderId, refundAmount, lineItemId, reason } = req.body as {
        orderId: string; refundAmount: number; lineItemId?: string; reason?: string;
    };
    ...
    if (activePolicy) {
        ...
        if (daysSince > activePolicy.daysWindow) {
            return res.status(403).json({ error: `Refunds allowed within ${activePolicy.daysWindow} days.` });
        }
    }

    // Find payment intent tied to order
    const pi = await PaymentIntent.findOne({ orderId: new Types.ObjectId(orderId) });
    if (!pi || !(pi.status === PaymentStatus.SUCCEEDED || pi.status === PaymentStatus.PARTIALLY_REFUNDED)) {
    ...
    if (lineItemId) {
        ...
        await CommissionLedger.create({ orderId: order._id, lineItemId: line._id, amount: -adjust, currency: order.currency, reason: 'refund' });
    } else {
