...
export async function commissionReport(req: Request, res: Response) {
    ...
    const orderAgg = await Order.aggregate([
        { $match: { 'lines.vendor': vendorId, createdAt: { $gte: startDate, $lte: endDate }, deleted: false } },
        { $unwind: '$lines' },
        { $match: { 'lines.vendor': vendorId } },
        { $group: { _id: '$lines.vendor', totalCommission: { $sum: '$lines.commission' }, count: { $sum: 1 } } }
    ]);
    ...
    if (format === 'csv') {
        ...
        res.setHeader('Content-Disposition', `attachment; filename="commission_report_${vendorId}_${start}_${end}.csv"`);
        const csvStream = parse({ headers: true });
        csvStream.pipe(res);
        csvStream.write({ vendor: report.vendor, totalCommission: report.totalCommission, totalCount: report.totalCount, start: report.start, end: report.end });
        csvStream.end();
        return;
    }
