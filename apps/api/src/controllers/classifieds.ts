...
export async function listClassifieds(_req: Request, res: Response) {
    const classifieds = await Classified.find({ deleted: false })
        .sort({ isFeatured: -1, featuredExpiry: -1, createdAt: -1 });
...
export async function createClassified(req: Request, res: Response) {
    ...
    if (images.length > maxImages) {
        return res.status(400).json({ error: `Your plan allows up to ${maxImages} images per classified.`, code: 'IMAGE_LIMIT_EXCEEDED' });
    }
    ...
    if (failedImages.length) {
        return res.status(400).json({ error: `Some images are missing the required watermark signature.`, failedImages });
    }
...
export async function deleteClassified(req: Request, res: Response) {
    const classified = await Classified.findByIdAndUpdate(req.params.id, { deleted: true }, { new: true });
    if (!classified) return res.status(404).json({ error: 'Not found' });
    res.json({ classified });
}
