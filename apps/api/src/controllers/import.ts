...
async function enqueueImportJob(type: string, rows: any[], userId: string) {
    ...
    for (const row of rows) {
        try {
            ...
            await AuditLog.create({ user: userId, action: 'import', resource: type, resourceId: row.id, details: row });
        } catch (err) {
...
export async function importCsv(req: Request, res: Response) {
    const { type } = req.body;
    if (!['product', 'service', 'classified'].includes(type)) return res.status(400).json({ error: 'Invalid type' });
