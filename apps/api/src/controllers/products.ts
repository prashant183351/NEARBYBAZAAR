...
export async function updateProduct(req: Request, res: Response) {
    const parse = ProductZ.safeParse(req.body);
    if (!parse.success) return res.status(400).json({ error: parse.error.errors });
    const product = await Product.findOneAndUpdate({ slug: req.params.slug }, parse.data, { new: true });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json({ product });
}

export async function deleteProduct(req: Request, res: Response) {
    const product = await Product.findOneAndUpdate({ slug: req.params.slug }, { deleted: true }, { new: true });
    if (!product) return res.status(404).json({ error: 'Not found' });
    res.json({ product });
}
