...
ClassifiedSchema.pre('validate', async function (next) {
  ...
  if (!this.expiresAt && (this as any).vendor) {
    ...
    try {
      // @ts-ignore
      plan =
        (await ClassifiedPlan.findOne({ vendor: (this as any).vendor, deleted: false })) ||
        (await ClassifiedPlan.findOne({ tier: 'Free', deleted: false }));
    } catch {}
    let days = 7;
    if (plan) {
...
ClassifiedSchema.pre('save', async function (next) {
  ...
  if ((this as any).isModified('title')) {
    ...
    const { slug, slugHistory } = updateSlugHistory(
      (this as any).slug,
      (this as any).slugHistory || [],
      generateSlug((this as any).title),
    );
    // @ts-ignore
    this.slug = slug;
    // @ts-ignore
