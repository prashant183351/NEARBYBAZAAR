...
VendorSchema.virtual('logoFallback').get(function (this: any) {
    ...
    const initials = (this.name || '').split(' ').map((n: string) => n[0]).join('').toUpperCase().slice(0, 2);
...
(VendorSchema.statics as any).findUniqueSlug = async function (self: any, base: string): Promise<string> {
    ...
    try {
        ...
        if (self.isModified && self.isModified('name')) {
            ...
            if (currentSlug && slug && currentSlug !== slug) {
                try { await recordSlugChange({ type: 'vendor', resourceId: self._id, oldSlug: currentSlug, newSlug: slug }); } catch { }
            }
