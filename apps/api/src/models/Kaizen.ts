...
export type KaizenType = z.infer<typeof KaizenZ> & Document & {
    owners?: Array<Types.ObjectId | string>;
    privateNotes?: Array<{ note: string; author?: Types.ObjectId | string; createdAt?: Date }>;
    attachments?: Array<{ url: string; filename?: string; mimetype?: string; size?: number; uploadedAt?: Date }>;
};
...
KaizenSchema.pre('save', function (next) {
    // @ts-ignore
    this.riceScore = ((this.reach || 1) * (this.impact || 1) * (this.confidence || 1)) / (this.effort || 1);
    next();
});
