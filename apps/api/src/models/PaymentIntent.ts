...
paymentIntentSchema.methods.canRefund = function (): boolean {
  return (
    (this.status === PaymentStatus.SUCCEEDED || this.status === PaymentStatus.PARTIALLY_REFUNDED) &&
 ...
 */
paymentIntentSchema.methods.capture = async function (amount?: number): Promise<IPaymentIntent> {
  if (!this.canCapture()) {
...
paymentIntentSchema.methods.refund = async function (
  ...
  if (amount > availableForRefund) {
    throw new Error(`Refund amount ${amount} exceeds available amount ${availableForRefund}`);
  }
...
paymentIntentSchema.methods.cancel = async function (): Promise<IPaymentIntent> {
  if (this.status !== PaymentStatus.PENDING && this.status !== PaymentStatus.REQUIRES_ACTION) {
