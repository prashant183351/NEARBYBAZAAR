...
describe('adTracking service', () => {
  ...
  const { calculateClickCost } = require('../src/services/adAuction');
  const { AdClick: MockAdClick } = require('../src/models/AdClick');
  const adTrackingModule = require('../src/services/adTracking');
  ...
  describe('recordImpression', () => {
    it('returns error if campaign not found', async () => {
      ...
      const result = await adTracking.recordClick({ campaignId: 'cid', placement: 'main', userId: 'uid' });
    ...
    it('returns error if duplicate click', async () => {
      ...
      Vendor.findById.mockResolvedValue({ wallet: { charge: jest.fn().mockResolvedValue(true) }, updateOne: jest.fn().mockResolvedValue(undefined) });
      const result = await adTracking.recordClick({ campaignId: 'cid', placement: 'main', userId: 'uid' });
      expect(result.success).toBe(false);
  ...
  expect(result.message).toMatch(/Duplicate click detected/);
    ...
    it('creates AdClick, updates vendor, returns success', async () => {
      const mockCampaign = { vendorId: 'vid', vendor: 'vid', _id: 'cid', product: 'pid', recordClick: jest.fn().mockResolvedValue(undefined), canServe: jest.fn().mockReturnValue(true) };
      AdCampaign.findById.mockImplementation(() => ({ populate: async () => mockCampaign }));
    ...
    it('returns error if exception thrown', async () => {
      ...
      await expect(adTracking.recordClick({ campaignId: 'cid', placement: 'main' }))
        .resolves.toMatchObject({ success: false, message: expect.stringMatching(/Internal error/) });
    });
    ...
    it('returns error if campaign budget exhausted', async () => {
      const mockCampaign = { vendorId: 'vid', vendor: 'vid', _id: 'cid', product: 'pid', recordClick: jest.fn().mockResolvedValue(undefined), canServe: jest.fn().mockReturnValue(false) };
      AdCampaign.findById.mockImplementation(() => ({ populate: async () => mockCampaign }));
    ...
    it('returns error if vendor wallet charge fails', async () => {
      ...
      const result = await adTracking.recordClick({ campaignId: '507f1f77bcf86cd799439012', placement: 'main', userId: 'uid' });
      expect(result.success).toBe(false);
    ...
    it('creates click with sessionId (no userId)', async () => {
      ...
      (MockAdClick as any).findOne.mockResolvedValue(null);
      Vendor.findById.mockResolvedValue({ wallet: { charge: jest.fn().mockResolvedValue(true) }, updateOne: jest.fn().mockResolvedValue(undefined) });
      const result = await adTracking.recordClick({ campaignId: 'cid', placement: 'main', sessionId: 'sess123' });
      expect(result.success).toBe(true);
      expect(result.clickId).toBe('clickid');
    });
  describe('recordImpression', () => {
    ...
    it('returns false if error thrown', async () => {
      AdCampaign.findById.mockImplementation(() => { throw new Error('fail'); });
      const result = await adTracking.recordImpression({ campaignId: 'cid', placement: 'main' });
      expect(result).toBe(false);
    });
  ...
  describe('recordConversion', () => {
    ...
    it('returns false if error thrown', async () => {
      (MockAdClick as any).findById.mockImplementation(() => { throw new Error('fail'); });
      const result = await adTracking.recordConversion('clickid', 'orderid');
      expect(result).toBe(false);
    });
