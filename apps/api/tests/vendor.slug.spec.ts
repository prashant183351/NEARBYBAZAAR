...
describe('Vendor slug behavior', () => {
    ...
    it('generates slug from name and enforces uniqueness', async () => {
        const v1 = await Vendor.create({ email: 'a@example.com', name: 'ACME Store', owner: new mongoose.Types.ObjectId() });
        const v2 = await Vendor.create({ email: 'b@example.com', name: 'ACME Store', owner: new mongoose.Types.ObjectId() });
    ...
    it('updates slug and keeps history on name change', async () => {
        const v = await Vendor.create({ email: 'c@example.com', name: 'Cool Shop', owner: new mongoose.Types.ObjectId() });
        const old = v.slug;
        v.name = 'Cooler Shop';
        await v.save();
