...
export default function StorePage({
    ...
    const description = vendor.description || `Shop products, services, and classifieds from ${vendor.name}${city ? ' in ' + city : ''}${category ? ' (' + category + ')' : ''}`;
    ...
    const businessSchema = generateLocalBusinessSchema({
        ...
        geo: (vendor as any).latitude && (vendor as any).longitude ? {
...
export const getServerSideProps: GetServerSideProps = async (context) => {
    ...
    try {
        ...
        const [productsRes, servicesRes, classifiedsRes] = await Promise.all([
            fetch(`${API_BASE}/v1/products?vendor=${vendor._id}&page=${currentTab === 'products' ? currentPage : 1}&limit=${limit}`),
            fetch(`${API_BASE}/v1/services?vendor=${vendor._id}&page=${currentTab === 'services' ? currentPage : 1}&limit=${limit}`),
            fetch(`${API_BASE}/v1/classifieds?vendor=${vendor._id}&page=${currentTab === 'classifieds' ? currentPage : 1}&limit=${limit}`),
        ]);
