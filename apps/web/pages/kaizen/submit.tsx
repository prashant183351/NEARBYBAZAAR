    ...
    return (
        <div style={{ maxWidth: 500, margin: '2em auto', padding: '2em', border: '1px solid #eee', borderRadius: 8 }}>
            ...
            <form onSubmit={handleSubmit}>
                <label>Title<br />
                    <input value={title} onChange={e => setTitle(e.target.value)} required style={{ width: '100%', marginBottom: 12 }} />
                </label>
                <label>Description<br />
                    <textarea value={desc} onChange={e => setDesc(e.target.value)} required style={{ width: '100%', marginBottom: 12 }} />
                </label>
            ...
            {status === 'error' && <div style={{ color: 'red', marginTop: 12 }}>Error submitting idea.</div>}
            {status === 'rate-limited' && <div style={{ color: 'orange', marginTop: 12 }}>Too many submissions. Please wait and try again.</div>}
        </div>
    );
}
