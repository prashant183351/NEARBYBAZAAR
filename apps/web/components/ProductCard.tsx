...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
        ...
        const showFomo = product.fomoEnabled && typeof product.stock === 'number' && typeof product.fomoThreshold === 'number' && product.stock <= product.fomoThreshold;
            ...
            <div className="bg-white rounded shadow p-4 flex flex-col relative">
                ...
                {tiers.length > 0 && (
                    <div className="mb-2">
                        ...
                        <ul className="text-xs text-gray-600">
                            {tiers.sort((a: { minQty: number }, b: { minQty: number }) => a.minQty - b.minQty).map((tier: { minQty: number; price: number }, i: number) => (
                                <li key={i}>
                ...
                {product.wholesaleOnly && (
                    <div className="mb-2 text-xs text-orange-700 bg-orange-50 p-2 rounded">
                        This product is available for wholesale buyers only. <span className="font-semibold">Please log in with a business account</span> to purchase.
                    </div>
                )}
                <div className="flex items-center mb-2">
                    ...
                    />
                    {minOrderQty > 1 && (
                        <span className="ml-2 text-xs text-gray-500">(Min {minOrderQty})</span>
                    )}
                </div>
                <button
